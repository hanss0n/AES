...
int main() {
    ...
    /*
    std::ifstream input("/Users/bjhans/CLionProjects/AES/aes_sample", std::ios::binary);
    ...
    unsigned char key[16];
    unsigned char message[bytes.size()-16];

    for (int i = 0; i < 16; ++i) {
        key[i] = bytes[i];
    }
    for (int i = 16; i < bytes.size(); ++i) {
        message[i-16] = bytes[i];
    }
