...
void encrypt(unsigned char *message, unsigned char *key, unsigned char * expanded_key) {
    ...
    for (int round = 1; round < num_rounds; ++round) {
        ...
        shift_rows(state);
        mix_columns(state);
        add_round_key(state, expanded_key + (16 * round)); //TODO: need to add to this
    }
    sub_bytes(state);
    shift_rows(state);
...
int main() {
    ...
    int lenOfPaddedMsg = originalLen;

    if (lenOfPaddedMsg % 16 != 0) {
        lenOfPaddedMsg = (lenOfPaddedMsg / 16 + 1) * 16;
    }
    unsigned char *paddedMessage = new unsigned char[lenOfPaddedMsg];

    for (int i = 0; i < lenOfPaddedMsg; ++i) {
        if (i >= originalLen) {
            paddedMessage[i] = 0;
        } else {
    ...
    for (int i = 0; i < lenOfPaddedMsg; i += 16) {
        encrypt(paddedMessage + i, key);
    }

    for (int i = 0; i < lenOfPaddedMsg; ++i) {

        //std::cout << std::hex << (unsigned int) paddedMessage[i] << " ";
